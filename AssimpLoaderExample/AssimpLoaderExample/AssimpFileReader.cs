...
// read the material textures into the model.
// read the deformers
// read the colors its got a weird setup for that with channels.
// fix the animations in the model make sure im setting them properly as well.
// fix the non animations in the model so bind pose maniupulation is possible.
// fix the shader the color is kinda screwed up the debug one is ok i think.
...
namespace AssimpLoaderExample
{
    public class AssimpFileReader
    {
        ...
        public AssimpFileReader(ContentManager Content, Effect defaulteffect)
        {
        ...
        public RiggedModel LoadAsset(string filePathorFileName, int defaultAnimatedFramesPerSecondLod)
        {
            ...
            try
            {
                ...
                scene = importer.ImportFile(filepathorname,
        ...
        private RiggedModel CreateModel(string filePath)
        {
            ...
            //Console.WriteLine("Scene number of animaton: " + scene.Animations.Count);
            Console.WriteLine();
            Console.WriteLine("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
            ...
            for (int i = 0; i < scene.Animations.Count; i++)
            {
                ...
                foreach (var anode in anim.NodeAnimationChannels)
                {
                    ...
                    for (int j =0; j < anode.PositionKeys.Count; j++) 
                    {
                        ...
                        if (startupconsoleinfo)
                            Console.WriteLine("       index[" + (j + "]").PadRight(5) + " Time: " + key.Time.ToString().PadRight(17) + " secs: "+ (key.Time/ anim.TicksPerSecond).ToString().PadRight(17) + "  Position: {" + key.Value + "}");
                    }
                    if (startupconsoleinfo)
                    ...
                    for (int j = 0; j < anode.RotationKeys.Count; j++)
                    {
                        ...
                        if (startupconsoleinfo)
                            Console.WriteLine("       index[" + (j + "]").PadRight(5) + " Time: " + key.Time.ToString().PadRight(17) + " secs: " + (key.Time / anim.TicksPerSecond).ToString().PadRight(17) + "  QRotation: {" + key.Value + "}");
                    }
                    if (startupconsoleinfo)
                    ...
                    for (int j = 0; j < anode.ScalingKeys.Count; j++)
                    {
                        ...
                        if (startupconsoleinfo)
                            Console.WriteLine("       index[" +( j + "]").PadRight(5)+" Time: " + key.Time.ToString().PadRight(17) + " secs: " + (key.Time / anim.TicksPerSecond).ToString().PadRight(17) + "  Scaling: {" + key.Value + "}");
                    }
                }
            }
        }
    ...
    public static class OpenAssimpToMgHelpers
    {
        ...
        public static string ToStringTrimed(this float v)
        {
            string d = "0.00";
            int pamt = 8;
            return (v.ToString(d).PadLeft(pamt));
        }
        public static string ToStringTrimed(this Assimp.Vector3D v)
        {
            string d = "0.00";
            int pamt = 8;
            return (v.X.ToString(d).PadLeft(pamt) + ", " + v.Y.ToString(d).PadLeft(pamt) + ", " + v.Z.ToString(d).PadLeft(pamt));
        }
        ...
        public static string ToStringTrimed(this Assimp.Quaternion q)
        {
            string d = "0.00#";
            int pamt = 8;
            return ("x: " + q.X.ToString(d).PadLeft(pamt) + "  y: " + q.Y.ToString(d).PadLeft(pamt) + "  z: " + q.Z.ToString(d).PadLeft(pamt) + "  w: " + q.W.ToString(d).PadLeft(pamt));
        }
    }
}
